...
const TodoItem = ({ todo, onToggle, onDelete, onEdit, onUpdatePriority }) => {
  ...
  return (
    <div
      className={`group p-4 rounded-xl border-2 transition-all duration-300 hover:shadow-md animate-fade-in ${todo.completed
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          {isEditing ? (
            <input
              ...
              className="w-full px-3 py-1.5 border-2 border-green-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-300 transition-all"
              autoFocus
            />
          ) : (
            <div>
              <span
                className={`text-gray-800 transition-all duration-300 ${todo.completed ? 'line-through text-gray-400' : ''
                  }`}
              >
                {todo.text}
              </span>
        ...
        <div className={`flex gap-2 transition-all duration-300 ${isEditing ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
          }`}>
          {isEditing ? (
            <>
              <button
